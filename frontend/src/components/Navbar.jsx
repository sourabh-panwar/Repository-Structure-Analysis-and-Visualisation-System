...
export default function Navbar({ repoPath, setRepoPath, handleScan, isScanning, showDeps, setShowDeps }) {
  return (
    <div style={{ padding: '15px 30px', backgroundColor: '#111418', borderBottom: '1px solid #2f3640', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ display: 'flex', gap: '15px', width: '50%' }}>
        ...
        />
        <button onClick={handleScan} disabled={isScanning} style={{ padding: '10px 20px', backgroundColor: isScanning ? '#7f8fa6' : '#3498db', color: 'white', border: 'none', borderRadius: '6px', cursor: isScanning ? 'not-allowed' : 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
