import os
from .ast_parser import parse_python_ast, parse_javascript_regex, get_python_profile, get_js_profile
...
def scan_project(directory_to_scan):
    ...
    nodes.append({"id": base_dir, "label": os.path.basename(base_dir) or base_dir, "type": "folder", "full_path": base_dir})
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for f in files:
            if f.endswith(ignore_exts) or "hot-update" in f: continue
    ...
    for node in nodes:
        if node["type"] == "file":
            deps = parse_python_ast(node["full_path"], file_map) if node["label"].endswith('.py') else parse_javascript_regex(node["full_path"], file_map)
            for target_path in set(deps): 
                edges.append({"id": f"dep_{node['id']}_{target_path}", "source": node["id"], "target": target_path, "edge_type": "dependency"})
